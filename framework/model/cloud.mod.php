...
function cloud_prepare() {
	...
	if(empty($_W['setting']['site']['key']) || empty($_W['setting']['site']['token'])) {
		//return error('-1', "您的程序需要在玩淘宝云服务平台注册你的站点资料, 来接入云平台服务后才能使用相应功能.");
	}
	return true;
}
...
function cloud_m_prepare($name) {
	...
	$pars['module'] = $name;
	$dat = cloud_request('http://addons.mitusky.com/meepo/system/gateway.php', $pars);
	if (is_error($dat)) {
		return $dat;
	}
...
function cloud_sms_send($mobile, $content, $postdata = array()) {
	...
	$response = cloud_request('http://addons.mitusky.com/meepo/system/gateway.php', $pars);
...
function cloud_download($path, $type = '') {
	...
	$dat = cloud_request('http://addons.mitusky.com/meepo/system/gateway.php', $pars, $headers, 300);
	if(is_error($dat)) {
	...
	if(is_error($ret)) {
		...
		return error(-1, '写入失败， 请给777权限和www用户组权限。');
	}
}
...
function _cloud_shipping_parse($dat, $file) {
	...
	if ($dat['content'] == 'blacklist') {
		return error(-1, '抱歉，您的站点已被列入云服务黑名单，云服务一切业务已被禁止，请联系玩淘宝客服！');
	}
	if (strlen($dat['content']) != 32) {
		return error(-1, 'Sorry！您的域名还未授权，请联系米兔授权，客服QQ:229364369');
	}
...
function cloud_cron_create($cron) {
	...
	$result = cloud_request('http://addons.mitusky.com/meepo/system/gateway.php', $pars);
