...
//					 $accessKeyId = $smsset['dayu_key'];//参考本文档步骤2
//					 $accessKeySecret = $smsset['dayu_secret'];//参考本文档步骤2
		             
                    include_once EWEI_SHOPV2_VENDOR.'dayu/api_sdk/aliyun-php-sdk-core/Config.php';
		            include_once EWEI_SHOPV2_VENDOR.'dayu/api_sdk/Dysmsapi/Request/V20170525/SendSmsRequest.php';
		            include_once EWEI_SHOPV2_VENDOR.'dayu/api_sdk/Dysmsapi/Request/V20170525/QuerySendDetailsRequest.php';
				    ...
				    //必填-短信签名
				    $request->setSignName($template['smssign']);
				    //必填-短信模板Code
				    $request->setTemplateCode($template['smstplid']);
				    ...
				    $dayuResult = (array) $acsResponse;
				    if (empty($dayuResult) || $dayuResult['Code'] !="OK") 
					{
						return array('status' => 0, 'message' => '短信发送失败(' . $dayuResult['Message'] . '/code: ' . $dayuResult['Code'] . '/sub_code: ' . $dayuResult['sub_code'] . ')');
		...
		if ($template['type'] == 'link') 
		{
						    ...
						    $pass = $smsset['link_pw'];
						    $url = $smsset['link_url'];
						    $mess = rawurlencode(mb_convert_encoding($msg, "gb2312", "utf-8"));
						    $num = $mobile;
						    $gateway = $url."?CorpID={$uid}&Pwd={$pass}&Mobile={$num}&Content={$mess}";       
						    $message = file_get_contents($gateway);
						    if($message<=0){
						    	return array('status' => 0, 'message' => '短信发送失败!错误码：'.$message);
						    }
		}
