...
class Pay_EweiShopV2Page extends MobileLoginPage 
{
	public function main() 
	{
        ...
        $body = $_W['uniacid'] . ':' . '0' . ':' . 'APP';
		$payinfo = array('orderid' => $orderid, 'credit' => $credit, 'alipay' => $alipay, 'wechat' => $wechat, 'cash' => $cash, 'money' => $order['price'], 'ordersn' => $order['ordersn'], 'body' => $body);
		if (is_h5app()) 
		{
	...
	public function complete() 
	{
     ...
     $paramData = array(
            ...
            'credit' => $order['deductcredit']
         );
       
    	 if (!buyOrder($paramData)) {
       		  show_json(0, '通信错误，下单失败');
         }
		...
		if ($type == 'credit') 
		{
			...
			$fee = floatval($ps['fee']);
			$result = m('member')->setCredit($openid, 'credit2', -$fee, array($_W['member']['uid'], $_W['shopset']['shop']['name'] . '消费' . $fee), 22);
			if (is_error($result)) 
			{
				if ($_W['ispost']) 
				{
