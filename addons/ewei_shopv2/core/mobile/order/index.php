...
class Index_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function main() 
	{
		...
		{  
			if($sj){
                include $this->template('member/merchOrder');
			}else{
	...
	public function get_list() 
	{
		...
		switch ($show_status) 
		{
			...
			break;
			case 1: $condition .= ' and (status=1 or (paytype=3 && status!=2)) ';
			break;
			case 2: $condition .= ' and status=2 ';
			break;
		...
		public function get_lists() 
	{
		...
		global $_GPC;
		$uniacid = $_W['uniacid'];
		$merchid = pdo_fetchcolumn('select id from ' . tablename('ewei_shop_merch_user') . ' where uniacid=:uniacid and openid=:openid limit 1',array(':openid' =>$_W['openid'],':uniacid' =>$_W['uniacid']));
		$pindex = max(1, intval($_GPC['page']));
		$psize = 50;
		$show_status = $_GPC['status'];
		$r_type = array('退款', '退货退款', '换货');
		$condition = ' and merchid=:merchid and ismr=0 and deleted=0 and uniacid=:uniacid ';
