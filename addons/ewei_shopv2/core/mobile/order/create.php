			...
			else 
			{
				...
				if (!(empty($optionid))) 
				{
					...
					if (!(empty($option))) 
					{
						...
						{
							$data['marketprice'] = $option['presellprice'];
						}
						else 
						{
	...
	public function submit() 
	{
		...
		if ($giftid) 
		{
			...
			if ($giftdata['giftgoodsid']) 
			{
				...
				$goods = array_merge($this->array_unique_fb(array_merge($goods, $gift)));
			}
		}
...
//		if (($ismerch == 0) || (($ismerch == 1) && (count($merch_array) == 1)))
		if (($ismerch == 0) || (($ismerch == 1) && $isverify))
		{
			$multiple_order = 0;
		}
		else 
		...
		if (0 < $deductcredit) 
		{
			m('member')->setCredit($openid, 'credit1', -$deductcredit, array('0', $_W['shopset']['shop']['name'] . '购物积分抵扣 消费积分: ' . $deductcredit . ' 抵扣金额: ' . $deductmoney . ' 订单号: ' . $ordersn), 22);
		}
		if (0 < $buyagainprice) 
		{
		...
		if (0 < $deductcredit2) 
		{
			m('member')->setCredit($openid, 'credit2', -$deductcredit2, array('0', $_W['shopset']['shop']['name'] . '购物余额抵扣: ' . $deductcredit2 . ' 订单号: ' . $ordersn), 22);
		}
		if (empty($virtualid)) 
		{
	...
	function array_unique_fb($array2D){
		$count=count($array2D);
		 foreach ($array2D as $k=> $v){
	          $repeat=1; 
		    for($i=$count;$i>=0;$i--){
	           if($v['goodsid']==$array2D[$i]['goodsid']){
	               $repeat+=1;
	           }
		    }
		    if($repeat > 2){
