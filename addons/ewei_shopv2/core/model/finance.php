...
class Finance_EweiShopV2Model 
{
	...
	public function RSAVerify($return_data, $public_key, $ksort = true) 
	{
        ...
        file_put_contents("b.txt", '$pkeyid--' . $pkeyid . "\r\n", FILE_APPEND);
		if (empty($pkeyid)) 
		{
			return false;
		}
        ...
        file_put_contents("b.txt", '$strdata--' . $strdata . "\r\n", FILE_APPEND);
		$rsasign = str_replace(' ', '+', $rsasign);
		$rsasign = base64_decode($rsasign);
        file_put_contents("b.txt", '$rsasign--' . $rsasign . "\r\n", FILE_APPEND);
		$rsaverify = openssl_verify($strdata, $rsasign, $pkeyid);
        file_put_contents("b.txt", '$rsaverify--' . $rsaverify . "\r\n", FILE_APPEND);
