...
class Member_EweiShopV2Model 
{
	...
	public function setCredit($openid = '', $credittype = 'credit1', $credits = 0, $log = array(), $moneyType = 0, $syncJava = 1, $member = array())
	{
		global $_W;
		load()->model('mc');
		$uid = mc_openid2uid($openid, $member['uniacid']);
        ...
        if($data['qy_open'] > 0 ){
        	if (!empty($syncJava)) {
	            global $_GPC;
	            $id = intval($_GPC['id']);
	            $profile = m('member')->getMember($openid, true);
	            if (!updateUserMoney($profile['mobile'], $credittype, $credits, $moneyType, $log)) {
	                //show_json(0, array('message' => '充值失败'));
	                exit('同步修改失败');
	            }
	        }
		...
		if (!(empty($uid))) 
		{
            ...
            //file_put_contents("c.txt", '$newcredit--' . $newcredit . "\r\n", FILE_APPEND);
			if ($newcredit <= 0) 
			{
				$newcredit = 0;
			}
			...
			$data = array('uid' => $uid, 'credittype' => $credittype, 'uniacid' => $member['uniacid'], 'num' => $credits, 'createtime' => TIMESTAMP, 'module' => 'ewei_shopv2', 'operator' => intval($log[0]), 'remark' => $log[1]);
			pdo_insert('mc_credits_record', $data);
			
		}
		else 
		{
			$value = pdo_fetchcolumn('SELECT ' . $credittype . ' FROM ' . tablename('ewei_shop_member') . ' WHERE  uniacid=:uniacid and openid=:openid limit 1', array(':uniacid' => $member['uniacid'], ':openid' => $openid));
			$newcredit = $credits + $value;
			if ($newcredit <= 0) 
			{
				$newcredit = 0;
			}
			pdo_update('ewei_shop_member', array($credittype => $newcredit), array('uniacid' => $member['uniacid'], 'openid' => $openid));
		}
	}
