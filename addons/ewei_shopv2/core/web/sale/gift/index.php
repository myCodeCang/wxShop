	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			else 
			{
				...
				foreach ($data['giftgoodsid'] as $key => $value) {
					$goodsHasoption = pdo_fetchcolumn('select hasoption from ' . tablename('ewei_shop_goods') . ' where uniacid=:uniacid and id=:id limit 1',array(':uniacid' =>$_W['uniacid'],':id'=>$value));
					if($goodsHasoption){
						show_json(0, '赠品不可启用商品规格！');
						break;
					}
				}
