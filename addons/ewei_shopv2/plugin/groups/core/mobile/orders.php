	...
	public function confirm() 
	{
		...
		try 
		{
			...
			if ($_W['ispost']) 
			{
				...
				{
					$order_address = pdo_fetch('select id,realname,mobile,address,province,city,area from ' . tablename('ewei_shop_member_address') . "\n\t\t\t\t\t" . 'where openid=:openid and deleted=0 and isdefault=1  and uniacid=:uniacid and id = ' . intval($_GPC['aid']) . ' ', array(':uniacid' => $uniacid, ':openid' => $openid));
				}
