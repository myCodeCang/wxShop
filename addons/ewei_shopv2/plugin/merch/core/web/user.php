...
class User_EweiShopV2Page extends PluginWebPage 
{
	...
	protected function post() 
	{
		...
		$tmobile=trim($_GPC['tmobile']);

        if($tmobile){
			$t_meber_info = pdo_fetch('select * from ' . tablename('ewei_shop_member') . ' where mobile=:mobile and istuiguang=1 ', array(':mobile' => $tmobile));
	         if (empty($t_meber_info)) 
					{    
						show_json(0, '该推广商不存在,请核实信息后进行操作!');
					}
				  $tid=$t_meber_info['id'];
		}else{
			if($_GPC['tid']){
		...
		$item = pdo_fetch("select smu.*,sm.mobile as tmobile ,sm.id as tid from ". tablename('ewei_shop_merch_user') . " smu left join ". tablename('ewei_shop_member') ." sm on smu.tid=sm.id WHERE smu.id=".$id." and smu.uniacid=".$_W['uniacid']." limit 1");
		...
		if ($_W['ispost']) 
		{
			...
			$data = array('uniacid' => $_W['uniacid'], 'merchname' => trim($_GPC['merchname']), 'salecate' => trim($_GPC['salecate']), 'realname' => trim($_GPC['realname']), 'mobile' => trim($_GPC['mobile']), 'address' => trim($_GPC['address']), 'tel' => trim($_GPC['tel']), 'lng' => $_GPC['map']['lng'], 'lat' => $_GPC['map']['lat'], 'accounttime' => strtotime($_GPC['accounttime']), 'accounttotal' => intval($_GPC['accounttotal']), 'groupid' => intval($_GPC['groupid']), 'cateid' => intval($_GPC['cateid']), 'isrecommand' => intval($_GPC['isrecommand']), 'remark' => trim($_GPC['remark']), 'status' => $status, 'desc' => trim($_GPC['desc1']), 'logo' => save_media($_GPC['logo']), 'payopenid' => trim($_GPC['payopenid']), 'payrate' => trim($_GPC['payrate'], '%'),'tid'=> $tid);
			if ($diyform_flag) 
			{
	...
	public function promote() 
	{
		...
		// 	$sortfield = 'u.jointime';
		// }
		$sql = 'select  u.*,g.groupname  from ' . tablename('ewei_shop_merch_user') . '  u ' . ' left join  ' . tablename('ewei_shop_merch_group') . ' g on u.groupid = g.id ' . ' where u.uniacid=:uniacid ' . $condition ;
		if (empty($_GPC['export'])) 
		{
			$sql .= ' limit ' . (($pindex - 1) * $psize) . ',' . $psize;
		}
