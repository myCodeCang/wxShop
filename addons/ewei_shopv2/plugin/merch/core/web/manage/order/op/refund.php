	...
	public function submit() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			else if ($refundstatus == 1) 
			{
				...
				if ($item['paytype'] == 1) 
				{
					m('member')->setCredit($item['openid'], 'credit2', $realprice, array(0, $shopset['name'] . '退款: ' . $realprice . '元 订单号: ' . $item['ordersn']), 22);
					$result = true;
				}
				...
				if (0 < $credits) 
				{
					m('member')->setCredit($item['openid'], 'credit1', -$credits, array(0, $shopset['name'] . '退款扣除购物赠送积分: ' . $credits . ' 订单号: ' . $item['ordersn']), 22);
				}
				if (0 < $item['deductcredit']) 
				{
					m('member')->setCredit($item['openid'], 'credit1', $item['deductcredit'], array('0', $shopset['name'] . '购物返还抵扣积分 积分: ' . $item['deductcredit'] . ' 抵扣金额: ' . $item['deductprice'] . ' 订单号: ' . $item['ordersn']), 22);
				}
				if (!(empty($refundtype))) 
				{
