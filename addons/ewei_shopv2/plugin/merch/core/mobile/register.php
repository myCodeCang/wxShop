...
class Register_EweiShopV2Page extends PluginMobileLoginPage 
{
	public function main() 
	{
		...
		if ($_W['ispost']) 
		{
            ...
            if($data['is_tg_mobiel']==1){
              if(empty($tmobile)){
              	show_json(0, '请填写推广商手机号！');
              }
            }
		        if($tmobile){
			...
			$data = array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid'], 'status' => 0, 'realname' => trim($_GPC['realname']), 'mobile' => trim($_GPC['mobile']), 'merchname' => trim($_GPC['merchname']), 'salecate' => trim($_GPC['salecate']), 'desc' => trim($_GPC['desc']),'tid'=>$tid);
			if ($template_flag == 1) 
			{
				$mdata = $_GPC['mdata'];
