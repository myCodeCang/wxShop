...
class Detail_EweiShopV2Page extends CreditshopMobilePage 
{
				...
				else 
				{
				 ...
				 //    //同步通知会员系统
				     $paramData = array(
				             ...
				             'orderid' => $orderid,
				             'orderMoney' => $goods['credit'] + $goods['money'] ,
	...
	public function lottery() 
	{
		...
		if ((0 < ($goods['money'] + $goods['dispatch'])) && ($log['paystatus'] < 1)) 
		{
			...
			{
				if($goods['dispatch']){
                 $goods['money']= $goods['money']+$goods['dispatch'];
				}
				m('member')->setCredit($openid, 'credit2', -$goods['money'], '积分商城消费扣除余额度 ' . $goods['money'], 22);
				$update['paystatus'] = 1;
			}
			if ($log['paytype'] == 1) 
			{
		...
		{
			m('member')->setCredit($openid, 'credit1', -$goods['credit'], '积分商城消费扣除积分 ' . $goods['credit'], 22);
			$update['creditpay'] = 1;
