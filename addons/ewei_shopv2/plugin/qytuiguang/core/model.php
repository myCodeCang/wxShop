...
if (!(class_exists('QytuiguangModel'))) 
{
	class QytuiguangModel extends PluginModel 
	{
		...
		public function getLevels($all = true, $default = false) 
		{
			...
			if ($default) 
			{
				$default = array('id' => '0', 'levelname' => (empty($_S['qytuiguang']['levelname']) ? '默认等级' : $_S['qytuiguang']['levelname']), 'bonus1' => $_W['shopset']['qytuiguang']['bonus1'], 'bonus2' => $_W['shopset']['qytuiguang']['bonus2'], 'bonus3' => $_W['shopset']['qytuiguang']['bonus3']);
		...
		public function sendMessage($openid = '', $data = array(), $message_type = '') 
		{
			...
			if (($message_type == TM_ABONUS_BECOME) && empty($usernotice['abonus_become'])) 
			{
				...
				$msg = array( 'keyword1' => array('value' => (!(empty($tm['becometitle'])) ? $tm['becometitle'] : '成为区域推广通知'), 'color' => '#73a68d'), 'keyword2' => array('value' => $message, 'color' => '#73a68d') );
