...
class Agent_EweiShopV2Page extends PluginWebPage 
{
	public function main() 
	{
		...
		foreach ($list as $k =>$sum_bouns) {
			$sum_bonus=pdo_fetchcolumn('select sum(money) from' .tablename('ewei_shop_member_log').' where type=8 and openid=:openid and uniacid=:uniacid ',array(':openid'=>$sum_bouns['openid'],':uniacid' => $_W['uniacid']));
			$sum_order=pdo_fetchcolumn('select count(id) from' .tablename('ewei_shop_member_log').' where type=8 and openid=:openid and uniacid=:uniacid ',array(':openid'=>$sum_bouns['openid'],':uniacid' => $_W['uniacid']));
			if($sum_bonus > 0){
                $list[$k]['bonus']=$sum_bonus;
			}else{
                 $list[$k]['bonus']=0;
			}
			
			if($sum_order > 0){
                 $list[$k]['sum_order']=$sum_order;
			}else{
                  $list[$k]['sum_order']=0;
			}
		}
		
		$total = pdo_fetchcolumn('select count(dm.id) from' . tablename('ewei_shop_member') . ' dm  ' . ' left join ' . tablename('ewei_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('ewei_shop_abonus_level') . ' l on l.id = dm.aagentlevel' . ' where dm.uniacid =' . $_W['uniacid'] . ' and dm.istuiguang =1  ' . $condition, $params);
	
		if ($_GPC['export'] == '1') 
		{
			ca('qytuiguang.agent.export');
			plog('qytuiguang.agent.export', '导出区域推广商数据');
			foreach ($list as &$row ) 
			{
	...
	public function delete() 
	{
		...
		{
			pdo_update('ewei_shop_member', array('istuiguang' => 0, 'aagentstatus' => 0), array('id' => $member['id']));
			plog('qytuiguang.agent.delete', '取消区域推广资格 <br/>区域推广信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
		}
	...
	public function query() 
	{
		...
		$condition = ' and uniacid=:uniacid and istuiguang=1';
		if (!(empty($kwd))) 
		{
		...
		include $this->template('qytuiguang/query');
	}
	public function check() 
	{
		...
		foreach ($members as $member ) 
		{
			...
			if ($status == 1) 
			{
				...
				plog('qytuiguang.aagent.check', '审核代理 <br/>代理信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			...
			else 
			{
				...
				plog('qytuiguang.agent.check', '取消审核 <br/>代理信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
		}
		show_json(1, array('url' => referer()));
	}
	public function aagentblack() 
	{
		...
		foreach ($members as $member ) 
		{
			...
			if ($aagentblack == 1) 
			{
				pdo_update('ewei_shop_member', array('istuiguang' => 1, 'aagentstatus' => 0, 'aagentblack' => 1), array('id' => $_GPC['id']));
				plog('qytuiguang.agent.aagentblack', '设置黑名单 <br/>区域推广信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
			else 
			{
				pdo_update('ewei_shop_member', array('istuiguang' => 1, 'aagentstatus' => 1, 'aagentblack' => 0), array('id' => $_GPC['id']));
				plog('qytuiguang.agent.aagentblack', '取消黑名单 <br/>区域推广信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
		}
		show_json(1, array('url' => referer()));
	}
