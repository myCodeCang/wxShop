...
class Cover_EweiShopV2Page extends PluginWebPage 
{
	public function main() 
	{
		global $_W;
		global $_GPC;
		$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'ewei_shopv2区域推广中心入口设置'));
		if (!(empty($rule))) 
		{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($keyword))) 
			{
				if ($keyword['name'] != 'ewei_shopv2区域推广中心入口设置') 
				{
					show_json(0, '关键字已存在!');
				}
			}
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'ewei_shopv2区域推广中心入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => intval($data['status']));
			...
			$cover_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => $this->modulename, 'title' => trim($data['title']), 'description' => trim($data['desc']), 'thumb' => save_media($data['thumb']), 'url' => mobileUrl('qytuiguang'));
			pdo_insert('cover_reply', $cover_data);
			plog('qytuiguang.cover.edit', '修改区域推广中心入口设置');
			show_json(1);
		}
		$url = mobileUrl('qytuiguang', NULL, true);
