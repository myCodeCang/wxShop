...
class Level_EweiShopV2Page extends PluginWebPage
{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				if ($id == 'default') {
					...
					$this->updateSet($set);
					plog('qytuiguang.level.edit', '修改代理商默认等级' . $updatecontent);
				}
				 else {
					...
					plog('qytuiguang.level.edit', '修改代理商等级 ID: ' . $id . $updatecontent);
				}
			}
			 else {
				pdo_insert('ewei_shop_abonus_level', $data);
				$id = pdo_insertid();
				plog('qytuiguang.level.add', '添加代理商等级 ID: ' . $id);
			}

			show_json(1, array('url' => webUrl('qytuiguang/level')));
		}
	...
	public function delete()
	{
		...
		foreach ($items as $item ) {
			...
			plog('qytuiguang.level.delete', '删除代理商等级 ID: ' . $id . ' 等级名称: ' . $item['levelname']);
		}
