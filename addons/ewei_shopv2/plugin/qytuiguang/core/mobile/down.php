...
class Down_EweiShopV2Page extends QytuiguangMobileLoginPage
{
	public function main()
	{
		...
		$total_merch = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_merch_user') . ' where tid=:tid and uniacid=:uniacid ',array(':tid'=>$member['id'],':uniacid' => $_W['uniacid']));
		$list=pdo_fetchall('select * from' .tablename('ewei_shop_merch_user').' where tid=:tid and uniacid=:uniacid ',array(':tid'=>$member['id'],':uniacid' => $_W['uniacid']));
		foreach ($list as $key => $value) {
			$single_merch_bonus=pdo_fetchcolumn('select sum(money) from' .tablename('ewei_shop_member_log').' where type=8 and merchid=:merchid and uniacid=:uniacid ',array(':merchid'=>$value['id'],':uniacid' => $_W['uniacid']));
            if($single_merch_bonus){
            	$list[$key]['single_merch_bonus']=$single_merch_bonus;
            }else{
            	$list[$key]['single_merch_bonus']=0;
            }
            $ordercount=pdo_fetchcolumn('select count(id) from' .tablename('ewei_shop_member_log').' where type=8 and merchid=:merchid and uniacid=:uniacid ',array(':merchid'=>$value['id'],':uniacid' => $_W['uniacid']));
            if($ordercount){
            	$list[$key]['ordercount']=$ordercount;
            }else{
            	$list[$key]['ordercount']=0;
            }
		}
        $openid = $_W['openid'];
        $total_price =pdo_fetchcolumn('select sum(money) from' .tablename('ewei_shop_member_log').' where type=8 and openid=:openid and uniacid=:uniacid ',array(':openid'=>$openid,':uniacid' => $_W['uniacid']));
        if(!$total_price){
             $total_price=0;
        }
