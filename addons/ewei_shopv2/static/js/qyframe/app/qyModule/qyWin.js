    ...
    parseA2OpenWin:function(){
        ...
        $(document).on('click', 'a',function(e){
           var $target = $(e.currentTarget);
            if($target.attr("appDirect")){
                e.preventDefault();
              
                api.closeToWin({
                    name:$target.attr("appDirect")
                });
                return;
            }
            ...
            e.preventDefault();
             if ($target.hasClass('back')) {
               ...
               var className = $target.attr("class");
                ...
                if(className){
                    className = className + Date.parse(new Date());
                }

                var merchid =   getPar("merchid");
                if (merchid) {
                   className = className+ merchid;
                }
             
                var appWinName =   getParByUrl(url,"appWinName");
               
                if (appWinName) {
                   className = appWinName+ merchid;
                 
                }
                
                //忽略app
                if($target.hasClass('ignapp')){
                      var ignoreCache = $target.attr('data-nocache') === 'true';
