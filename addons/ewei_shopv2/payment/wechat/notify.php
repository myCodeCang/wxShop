...
class EweiShopWechatPay 
{
				...
				if (method_exists($site, $method)) 
				{
					...
					if ($result) 
					{
						...
						if($contact_order_info['isparent']){
							pdo_update('ewei_shop_order', array('paytype' => 22,'merchshow' => 1, 'apppay' => ($this->isapp ? 1 : 0), 'transid' => $this->post['trade_no'], 'status'=> 1), array('ordersn' => $log['tid'], 'uniacid' => $log['uniacid']));
							pdo_update('ewei_shop_order', array('paytype' => 22,'merchshow' => 0, 'apppay' => ($this->isapp ? 1 : 0), 'transid' => $this->post['trade_no'], 'status'=> 1), array('parentid' => $contact_order_info['id'], 'uniacid' => $log['uniacid']));
						}else{
	...
	public function publicMethod() 
	{
		...
		if (is_array($this->setting['payment']) || ($this->set['pay']['weixin_jie'] == 1) || ($this->set['pay']['weixin_sub'] == 1) || ($this->set['pay']['weixin_jie_sub'] == 1) || ($this->get['trade_type'] == 'APP')) 
		{
			...
			else if ($this->get['trade_type'] == 'APP')
			{
				$this->isapp = true;
				...
				$wechat = array('version' => 1, 'key' => 'liweijia1314haha123hehe321haha51', 'signkey' => 'liweijia1314haha123hehe321haha51', 'appid' => 'wx45f2964378ca509e', 'mchid' => '1483846582');
			}
			if (!(empty($wechat))) 
			{
				ksort($this->get);
