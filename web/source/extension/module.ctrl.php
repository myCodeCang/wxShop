...
if($do == 'installed') {
	...
	if (!empty($modules)) {
		foreach ($modules as $mid => $module) {
			...
			$modules[$mid]['official'] = empty($module['issystem']) && (strexists($module['author'], 'WeEngine Team') || strexists($module['author'], '米兔团队'));
...
if($do == 'permission') {
	...
	if (empty($module)) {
		message('你访问的模块不存在. 或许你愿意去米兔云服务平台看看. ', 'http://v2.addons.we7.cc/web/index.php?keyword=' . $_GPC['title']);
	}
...
<manifest xmlns="http://www.qynetwork.io" versionCode="{$versions}">
...
class {$name}ModuleProcessor extends WeModuleProcessor {
	public function respond() {
		...
		//这里定义此模块进行消息处理时的具体过程, 请查看米兔文档来编写你的代码
	}
}
TPL;
	}
...
class {$name}ModuleReceiver extends WeModuleReceiver {
	public function receive() {
		...
		//这里定义此模块进行消息订阅时的, 消息到达以后的具体处理过程, 请查看米兔文档来编写你的代码
	}
}
TPL;
	}
